(function() {
   ...
   document.head.appendChild(meta);
      ...
      function openSpectateTab() {
        try {
          ...
          <div class="player-team-section">
            ...
            <div class="content player-list" style="display: none;">
              ${buildUsersHTML()}
            </div>
            <div class="collapsible active" onclick="toggleCollapse(this)">
              Team <span class="arrow" style="transform: rotate(90deg);">▶</span>
            </div>
            <div class="content team-list" style="display: block;">
              ${buildTeamsHTML()}
            </div>
          </div>
    ...
    if (parent && parent.Texture && parent.Texture.customSkinMap) {
      ...
      console.log("[DEBUG] Skin Map:", skinMap);
      ...
      if (typeof skinMap.forEach === "function") {
        skinMap.forEach((value, key) => {
          ...
          const skinURL = value;
          if (!nick || nick.trim() === "") {
            console.log("[DEBUG] Skipped empty nick for key:", key);
            return;
          }
          // Clean up the nickname by removing trailing numbers and extra whitespace
          nick = nick.replace(/[0-9]+$/, '');
          nick = nick.replace(/[\s\u200B-\u200D\uFEFF]/g, '');
          console.log("[DEBUG] Processed team player:", nick, skinURL);
      ...
      console.log("[DEBUG] Final team players array:", newArray);
    } else {
      teamsHTML = `<div class="player">No spectator data available</div>`;
      console.log("[DEBUG] No customSkinMap available.");
      return teamsHTML;
    }
    ...
    if (newArray.length === 0) {
      ...
      newArray.forEach(item => {
        teamsHTML += `
          <div class="player">
            <div class="player-info" onclick="copyPlayerInfo(event, this)">
              <img src="${item._skinURL}" alt="User" crossorigin="anonymous" onerror="console.error('Failed to load image:', this.src)" />
  ...
  setInterval(updateSpec, 3000);
      ...
      window.toggleCollapse = function(element) {
          try {
            ...
            if (bothOpen) {
              ...
              playerList.style.maxHeight = '240px'; // Default when one or none open
              teamList.style.maxHeight = '240px';
            }
          } catch (err) {
      ...
      document.addEventListener('keydown', function(e) {
        try {
          if (e.key === '0') {
            // Block the "0" event from reaching the game.
      ...
      }, true); // Capturing phase ensures our listener fires first.
      
      function toggleSpectateTab() {
        let panel = document.getElementById('spectateTab');
        if (panel) {
          ...
          if (panel.style.left === '0' || panel.style.left === '0px') {
            ...
            setTimeout(() => {
              try {
                ...
                console.error("[toggleSpectateTab] Error removing spectate panel:", err);
              }
            }, 500);
          } else {
            // If the panel exists but is closed, open it.
            openSpectateTab();
          }
        } else {
          // If no panel exists, open it.
          openSpectateTab();
        }
      }
